    ...
    try:
        ...
        if os.path.exists(SESSION_FILE):
            try:
                ...
                log(f"⚠️ Login failed (Bad password reported by Instagram): {e}")
                log("⚠️ Instagram ने आपके IP / session को block किया हो सकता है; "
                    "कृपया mobile network / VPN से try करें या कुछ देर बाद पुनः प्रयास करें.")
                return
            except Exception as e:
                log(f"⚠️ Old session invalid, deleting and trying fresh login: {e}")
                try:
                ...
                try:
                    ...
                    log("✅ Fresh session login successful and saved.")
                except BadPassword as e2:
                    log(f"⚠️ Fresh login भी fail: {e2}")
                    log("⚠️ Password सही हो सकता है लेकिन IP / device Instagram द्वारा blacklist है; "
                        "IP बदलकर या official app से login check करें.")
                    return
        else:
            ...
            try:
                ...
                log("⚠️ अगर password बिल्कुल सही है फिर भी ये error आए तो "
                    "अपना IP बदलकर (जैसे mobile data / VPN) दोबारा कोशिश करें.")
                return
    except Exception as e:
        log(f"⚠️ Login failed (unknown error): {e}")
        log("⚠️ यह अक्सर IP blacklist या security challenge की वजह से होता है; "
            "कुछ समय बाद या अलग नेटवर्क से login करें.")
        return
                ...
                try:
                    for msg in welcome_messages:
                        ...
                        try:
                            ...
                            log("⚠️ Session expired during sending. Trying to re-login...")
                            try:
                                if os.path.exists(SESSION_FILE):
                                    cl.load_settings(SESSION_FILE)
                                cl.login(username, password)
                                cl.direct_send(final_msg, thread_ids=[gid])
                                log("✅ Re-login successful; message sent after session refresh.")
                            except Exception as e_lr:
                                log(f"🛑 Re-login failed: {e_lr}")
                                log("⚠️ कृपया नया session बनाने के लिए /start दोबारा चलाने से पहले "
                                    "IP बदलें या कुछ देर wait करें.")
                                STOP_EVENT.set()
                                break
...
def start_bot():
    ...
    BOT_THREAD = threading.Thread(
        target=run_bot,
        args=(username, password, welcome, group_ids, delay, poll, custom_name),
        daemon=True
    )
    BOT_THREAD.start()
